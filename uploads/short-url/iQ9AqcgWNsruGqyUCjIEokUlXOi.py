...
def get_play_list(start_url, cid, quality):
    ...
    if'durl'not in html:return None
...
#  下载视频
def down_video(video_list, title, start_url, page,folder):
    num = 1
    print('[正在下载P{}段视频,请稍等...]:'.format(page) + title)
    currentVideoPath = os.path.join(folder, title)  # 当前目录作为下载目录
    if os.path.exists(currentVideoPath):return None
    for i in video_list:
        ...
        try:
            ...
            os.system('rm -rf "%s"'%currentVideoPath)
    return True

# 合并视频
def combine_video(video_list, title,folder):
    currentVideoPath = os.path.join(folder, title)  # 当前目录作为下载目录
    if not os.path.exists(currentVideoPath):
        os.makedirs(currentVideoPath)
    if len(video_list) >= 2:
...
def download(folder,link):
    ...
    if'message'in html:
        if html['message']=='稿件不可见':return None
    try:
        if not html['data']:return None
    except:return None
    data = html['data']
    ...
    for item in cid_list:
        ...
        if video_list:
            ...
            dl=down_video(video_list, title, start_url, page,folder)
            if dl:combine_video(video_list, title,folder)
    
        # 如果是windows系统，下载完成后打开下载目录
            currentVideoPath = folder  # 当前目录作为下载目录
